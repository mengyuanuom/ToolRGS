...
def grasp_relative_offset_scale(rectangle):
    """Return the half diagonal used to normalize an Offset V2 target."""
...
def resample_grasp_geometry(
    ...
    Dense grasp decoding first locates a quality peak.  Center-offset refinement
    can move that peak, so retaining angle and width from the old location makes
    the final rectangle internally inconsistent.  This helper preserves the
    refined center and height while bilinearly decoding geometry at the new
    center.  The doubled-angle representation keeps parallel-jaw periodicity.
    """
