"""MapleGrasp mask-guided dense grasp model.

This implementation adapts the official CROG-based MapleGrasp head to the
shared ToolRGS five-map contract.  It keeps the paper's detached predicted-mask
gating between segmentation and grasp prediction while avoiding fixed spatial
sizes and ground-truth-mask leakage during evaluation.
"""
...
class MapleGrasp(CROG):
    ...
    def __init__(self, cfg):
        ...
        self.grasp_loss_weight = float(getattr(cfg, "maple_grasp_loss_weight", 1.0))
    ...
    def forward(
        ...
        if self.maple_stage != "segmentation":
            ...
            for name, value in zip(("m_qua", "m_sin", "m_cos", "m_wid"), grasp_losses):
