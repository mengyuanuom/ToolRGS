...
ToolRGS now uses the second-stage MMDetection-style architecture. Shared
...
The preferred layout is `configs/_base_/{datasets,models,schedules,runtime}`
plus small experiment files such as
...
remain supported. The canonical dataset import is now
`toolrgs.datasets.build_dataset`; it temporarily bridges to the historical
implementation in `utils.data_builder`.
