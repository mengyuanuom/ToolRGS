...
2. 从配置的 GitHub Release 下载全部可选抓取权重（`V3-DROG-OFF-V2`、`V3-MambaGrasp`、`V3-CROG`、DROG-OFF V2 与 aligned CROG V2）；
...
`V3-DROG-OFF-V2`、`V3-MambaGrasp`、`V3-CROG`、DROG-OFF V2 与 aligned CROG V2。三个
V3 名称均以前缀 `V3-` 开头并排在列表前面。切换模型只重载抓取网络，不会重启相机、检测器或
机械臂连接。

V3 模型的抓取质量/尺寸激活方式随 profile 固定绑定，不能混用：
`V3-DROG-OFF-V2` 与 `V3-MambaGrasp` 的质量和尺寸均使用 `sigmoid`，
`V3-CROG` 的尺寸使用 `clamp`。权重包元数据和
