...
ToolRGS includes the official ETRG-A RGB-D model blocks under `model/etrg/`
...
The configured root must be the directory directly containing `refer/`. Check
...
sizes are per GPU. The supplied profiles target two 24 GB RTX 3090 GPUs; lower
both batch values if memory is insufficient.

ETRG requires `DATA.with_depth: true`. ToolRGS fails before dataset construction
...
The upstream result uses a dedicated depth preprocessing path. ToolRGS matches
