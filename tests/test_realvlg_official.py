...
class RealVLGDatasetAdapterTest(unittest.TestCase):
    ...
    def test_cuda_native_v3_profile_matches_npu_ablation(self):
        path = (
            ROOT
            / "config"
            / "realvlg"
            / "native_v3_lora_r24_12l_bs128_e24.yaml"
        )
        ...
        self.assertEqual(config["TRAIN"]["native_lora_rank"], 24)
        self.assertEqual(config["TRAIN"]["native_lora_alpha"], 48.0)
        self.assertEqual(
            config["TRAIN"]["native_visual_lora_layers"], list(range(12))
        )
        self.assertEqual(
            config["TRAIN"]["native_text_lora_layers"], list(range(12))
        )
        self.assertEqual(config["TRAIN"]["batch_size"], 128)
        self.assertTrue(config["TRAIN"]["batch_size_is_global"])
        self.assertEqual(config["TRAIN"]["epochs"], 24)
        self.assertFalse(config["TRAIN"]["amp"])
        self.assertEqual(config["Distributed"]["dist_backend"], "nccl")
